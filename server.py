...
# 初始化全局变量
converter = _Latex2Sympy(is_real=False)
...
def get_latex():
    try:
        return jsonify({
            'data': latex2latex(request.json['data'], variable_values=converter.var, is_real=converter.is_real),
            'error': ''
        })
...
def get_matrix_raw_echelon_form():
    try:
        parsed_math = latex2sympy(request.json['data'], variable_values=converter.var, is_real=converter.is_real)
        return jsonify({
            'data': latex(parsed_math.subs(converter.variances).rref()[0]),
            'error': ''
        })
    except Exception as e:
        return jsonify({
